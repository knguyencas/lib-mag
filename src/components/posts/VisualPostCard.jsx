...
const VisualPostCard = ({ post, onClick }) => {
  ...
  const loadMyVote = async () => {
    const vote = await voteService.getMyVote('visual_post', post.post_id);
  ...
  const handleHeartClick = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await voteService.likeVisualPost(post.post_id);
  ...
  const authorUsername = post.author_username || post.author?.username || 'user1';
  const imageUrl = post.image?.url || post.image_url || post.imageUrl || '';
